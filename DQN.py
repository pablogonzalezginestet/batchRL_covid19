...
class DQN(object):
    ...
    #
    def train(self,df,state_features, it=0):
        sum_q_loss=0
...
def train_DQN(df, val_df,batch_size ,num_actions, state_dim, state_features, device, parameters, behav_pol):
    # For saving files
...
def eval_policy_wis(val_df,state_features,policy, behav_pol,device):
    ...
    for idx,entity in enumerate(country_idx):
        ...
        p_new_traj = (F.softmax(policy.Q(state_idx), dim=-1)).gather(1,action_idx)
...
def eval_policy_dr(val_df,state_features,policy, behav_pol,device):
    ...
    for idx,entity in enumerate(country_idx):
        ...
        for t in range(len_traj):
            V_st = (F.softmax(policy.Q(state_idx)[len_traj-1 - t,:]).detach()*policy.Q(state_idx)[len_traj-1 - t,:].detach()).sum()
            V_dr_agent = V_st + torch.tensor(ratio_prob[len_traj-1 - t])*( reward_traj[len_traj-1 - t]+ .99 * V_dr_agent - policy.Q(state_idx)[len_traj-1 - t,:].detach().gather(0,action_idx[len_traj-1 - t]) )
            V_dr_gov = V_st + torch.tensor(1)*( reward_traj[len_traj-1 - t]+ .99 * V_dr_gov - policy.Q(state_idx)[len_traj-1 - t,:].detach().gather(0,action_idx[len_traj-1 - t]) )
        V_DR_list_agent.append(np.clip(V_dr_agent.cpu(),-500, 500))
        V_DR_list_gov.append(V_dr_gov.cpu())
        q_agent, action_agent = policy.Q(state_idx).max(1, keepdim=True)
...
def train_DQN_policy(train_df, val_df, state_dim, state_features, num_actions, batch_size, device, policy_save_file, policy_eval_save_file_wis,
    ...
    train_DQN(train_df,val_df,batch_size, num_actions, state_dim,state_features, device, params, behav_pol)
...
def run_dqn():
    ...
    for train_index, test_index in kf.split(idx_country):
        ...
        countries_file = './srl_test/results/rewards_gap5/states_t/countries_test_set' + f'_rnd_state{v_fold}.npy'
        np.save(countries_file,np.unique(val_df['Entity']))
        #
        loss_train_save_file = './srl_test/results/rewards_gap5/states_t/loss_train' + f'_rnd_state{v_fold}'
        policy_eval_save_file_wis = './srl_test/results/rewards_gap5/states_t/DQN_policy_eval_wis' + f'_rnd_state{v_fold}'
        policy_eval_save_file_dr = './srl_test/results/rewards_gap5/states_t/DQN_policy_eval_dr' + f'_rnd_state{v_fold}'
        policy_save_file = './srl_test/results/rewards_gap5/states_t/DQN_policy_data' + f'_rnd_state{v_fold}'
        q_rl_save_file = './srl_test/results/rewards_gap5/states_t/Q_values' + f'_rnd_state{v_fold}'
        action_rl_save_file = './srl_test/results/rewards_gap5/states_t/action_chosen' + f'_rnd_state{v_fold}'
        #action_chosen_iter_save_file = './srl_test/results/rewards_gap5/states_t/action_chosen_iter' + f'_rnd_state{v_fold}'
        #
        train_DQN_policy(train_df,val_df,state_dim,state_features,num_actions,batch_size,device,policy_save_file, policy_eval_save_file_wis,policy_eval_save_file_dr,loss_train_save_file,q_rl_save_file,action_rl_save_file ,behav_policy_file_wDemo)
        v_fold += 1
