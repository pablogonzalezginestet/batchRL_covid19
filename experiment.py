...
class Experiment(object): 
    ...
    def train_autoencoder(self):
        ...
        for epoch in range(epoch_0, self.autoencoder_num_epochs):
            ...
            for ii, (dem, ob, ac, l, t, rewards, idx,dones) in enumerate(self.train_loader):
                ...
                #else:
                loss_pred, mse_loss, _ = self.container.loop(ob, dem, ac, l, max_length, self.context_input, corr_coeff_param = self.corr_coeff_param, device=device, autoencoder = self.autoencoder)   
        ...
        with torch.no_grad():
            ...
            #for i_set, loader in enumerate([self.train_loader, self.val_loader, self.test_loader]):
                ...
                for dem, ob, ac, l, t, rewards, idx, dones in loader:
                    ...
                    if i_set == 1:  # If we're evaluating the models on the test set...
                    ...
                    for i_trans in range(cur_rep.shape[0]):
                        ...
                        if self.context_input:
                            self.replay_buffer.add(cur_rep[i_trans].numpy(), cur_actions[i_trans].argmax().item(), next_rep[i_trans].numpy(), cur_rewards[i_trans].item(), done[i_trans].item(), torch.cat((cur_obs[i_trans],cur_dem[i_trans]),dim=-1).numpy(), torch.cat((next_obs[i_trans], next_dem[i_trans]), dim=-1).numpy())
                        else:
                            self.replay_buffer.add(cur_rep[i_trans].numpy(), cur_actions[i_trans].argmax().item(), next_rep[i_trans].numpy(), cur_rewards[i_trans].item(), done[i_trans].item(), cur_obs[i_trans].numpy(), next_obs[i_trans].numpy())
    ...
    def train_dBCQ_policy(self, pol_learning_rate=1e-3):
        ...
        #test_representations = torch.load('./srl_test/results/test/rnn_data/test_representations.pt')
        test_demog, test_states, test_interventions, test_lengths, test_times, test_rewards, test_dones = torch.load('./data/rewards_new_cases/gap_5/test_set_tuples')
        ...
        train_dBCQ(replay_buffer, self.num_actions, self.hidden_size, self.device, params, behav_pol, self.context_input, test_representations ,test_demog, test_states, test_interventions, test_lengths, test_times, test_rewards,test_dones) # this is trained using the representation
    ...
    def train_DQN_policy(self, pol_learning_rate=1e-3):
        ...
        test_representations = torch.load(self.test_representations_file)
        # Initialize a dataloader for policy evaluation (will need representations, observations, demographics, rewards and actions from the test dataset)       
        test_demog, test_states, test_interventions, test_lengths, test_times, test_rewards, test_dones = torch.load('./data/rewards_new_cases/gap_5/test_set_tuples')
        ...
        # Run dBCQ_utils.train_dBCQ
        train_DQN_rep(replay_buffer, self.num_actions, self.state_dim + self.context_dim, self.device, params, behav_pol, self.context_input, test_representations ,test_demog, test_states, test_interventions, test_lengths, test_times, test_rewards,test_dones) # this is trained using the representation

    ####################################
    '''
    def run_behavcloning(self,  num_epochs = 10):
        # Construct training and validation loops
        #validation_losses = []
        # Initialize and load the training and validation buffers to populate dataloaders
        device = self.device
        obs_state_dim = self.state_dim + (self.context_dim if self.context_input else 0)
        #train_buffer = ReplayBuffer(self.hidden_size,self.minibatch_size, 200000, device)
        train_buffer = ReplayBuffer(obs_state_dim,self.minibatch_size, 200000, device,encoded_state=True)
        train_buffer.load(self.buffer_save_file, bootstrap=True)
        states = train_buffer.obs_state[:train_buffer.crt_size]
        #print(train_buffer.action)
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=self.minibatch_size, shuffle=True)

        training_losses = []
        training_iters = 0
        eval_frequency = 100

        storage_dir = './behavcloning/test/'
        num_nodes = 128
        learning_rate = 1e-4
        weight_decay = 0.1
        optim_type= 'adam'

        BC_network = BehaviorCloning(obs_state_dim, self.num_actions, num_nodes, learning_rate, weight_decay, optim_type, device)
